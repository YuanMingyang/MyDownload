...
- (void)download:(NSString *)urlString progress:(void (^)(TaskProgressInfo *))progressInfo complete:(void (^)(NSString *, NSError *))complete enableBackgroundMode:(BOOL)backgroundMode{
...
- (TaskProgressInfo *)progressInfoIfFileExsit:(NSString *)urlString{
    ...
    if ([self fileExistsWithName:fileName] || [self fileHasAddToUrlArray:urlString]) {
        ...
        if ([self fileExistsWithName:fileName]) {
            ...
            progressInfo.path = [self getFilePathWith:urlString];
        }else{
...
-(NSString *)getFilePathWith:(NSString *)urlString{
    NSString *base = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingString:@"/"];
    
    NSString *fileName = [[DownloadManager shareManager]  getFileName:urlString];
    NSString *path = [NSString stringWithFormat:@"%@%@",base,fileName];
    return path;
}

- (NSString *)getFileName:(NSString *)urlString{
    NSString *fileNameWithoutType = [self md5:urlString];
    NSString *fileName = [NSString stringWithFormat:@"%@.%@",fileNameWithoutType,[urlString pathExtension]];
    return fileName;
}
/**
 *  返回文件类型
